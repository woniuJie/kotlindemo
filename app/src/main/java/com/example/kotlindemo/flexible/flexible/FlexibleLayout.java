package com.example.kotlindemo.flexible.flexible;
...
import com.example.kotlindemo.flexible.flexible.callback.OnPullListener;
import com.example.kotlindemo.flexible.flexible.callback.OnReadyPullListener;
import com.example.kotlindemo.flexible.flexible.callback.OnRefreshListener;
import com.example.kotlindemo.flexible.flexible.util.PullAnimatorUtil;
        ...
        if (isEnable && isHeaderReady() && isReady()) {
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (diffX > 0) {
                        diffX = Math.max(0, diffX - mTouchSlop);
                    } else {
                        diffX = Math.min(0, diffX + mTouchSlop);
                    }
                    if (diffY > 0) {
                        diffY = Math.max(0, diffY - mTouchSlop);
                    } else {
                        diffY = Math.min(0, diffY + mTouchSlop);
                    }

                    if (diffY > 0 && Math.abs(diffY) * 0.5f > Math.abs(diffX)) {
